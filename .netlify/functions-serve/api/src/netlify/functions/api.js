...
function getLoad(parse8, render3) {
  return function load2(content, options, isDocument2 = true) {
    ...
    function initialize(selector, context, root3 = initialRoot, opts) {
      ...
      const r2 = typeof root3 === "string" ? [parse8(root3, options2, false, null)] : "length" in root3 ? root3 : [root3];
...
async function get_addresses(postcode) {
  ...
  const $2 = esm_default2.load(body);
  const gov_partial_url = "https://find-energy-certificate.service.gov.uk";
  let links = $2("a.govuk-link");
  links.each((i2, link) => {
    let href = $2(link).attr("href");
    if (href.startsWith("/energy-certificate")) {
      let address = $2(link).text().trim();
...
async function get_data_from_certificate(url) {
  ...
  if (response.ok) {
    const body = await response.text();
    const $2 = esm_default2.load(body);
    const links = $2("dl.govuk-summary-list");
    const floor_area_node = $2("#main-content > div > div.govuk-grid-column-two-thirds.epc-domestic-sections > div.govuk-body.epc-blue-bottom.printable-area.epc-box-container > dl > div:nth-child(2) > dd");
    const floor_area_txt = floor_area_node.text().trim();
    console.log("Floor Area: ", floor_area_txt);
    const epc_space_heating_node = $2("#current-space-heating-demand");
    const epc_space_heating_txt = epc_space_heating_node.text().trim();
    console.log("Space heating: ", epc_space_heating_txt);
    const valid_until_node = $2("#main-content > div > div.govuk-grid-column-two-thirds.epc-domestic-sections > div.govuk-body.epc-blue-bottom.printable-area.epc-box-container > div > div.epc-extra-boxes > p:nth-child(1) > b");
...
async function fetch_epc_data({ postcode, certificate }) {
  try {
    ...
    if (postcode) {
      ...
      const data2 = await get_data_from_certificate(url_cert);
      if (Object.keys(data2).length === 0) {
        return { "status": 404, "error": `certificate <${certificate}> is not valid` };
      } else {
        return { "status": 200, "result": data2 };
      }
    } else {
